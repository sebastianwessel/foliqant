...
Status: active implementation specification. The project owner narrowed the service
on 2026-09-21 to a simple in-memory pipeline from one accepted input through
configured steps to one terminal result. This scope supersedes earlier durable,
broker, child-job, application-authentication and packaged HTTP transport plans.

## Scope and authority

The service is an independent Python 3.12 package built with PydanticAI, Pydantic,
...
Existing native decision contracts are reused from `foliqant-decisions` without
changing their wire shapes or semantic validator. Model training, data curation,
paid inference, customer data and publishing remain outside this service scope.

## Architecture and reuse

`service/` is a separate project and environment. Its runtime dependencies exclude
...
failure and cancellation preserve the accepted input and completed step records.
...
| --- | --- |
| `SERVICE-CONTRACTS` | Strict envelopes/results, independent optional identity, W3C carrier, generated schema drift and no model-tooling import |
| `SERVICE-COMPILER` | Safe deterministic bundle compilation, graph/dataflow/schema checks, duplicate/path escape rejection and no endpoint I/O |
| `SERVICE-RUNTIME` | End-to-end in-memory decision/LLM/MCP/handler/finish execution, bounded concurrency, cancellation and concurrent state isolation |
| `SERVICE-MCP` | Current SDK HTTP/stdio behavior, OAuth isolation, declared catalog/schema checks, budgets, authorization and protected context propagation |
| `SERVICE-PRIVACY` | Secret/PII sentinel checks across safe logs and optional observations; telemetry failure remains nonfatal |
| `SERVICE-DX` | Locked install, public schema drift, CLI/example execution, documentation/skill checks and independent review |
...
the reviewed package APIs; `service/uv.lock` is the exact dependency authority.
