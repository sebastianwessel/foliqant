...
flowchart TD
  ...
  subgraph security["security"]
    direction TB
    security__assess{{"assess<br/>decision · choice"}}
    security__confirm(["confirm<br/>llm"])
    security__assess --> security__confirm
  end
  outcome_completed__(["completed"])
  outcome_needs_review__(["needs_review"])
  start__ --> security
  security --> outcome_completed__
  security -.->|"review"| outcome_needs_review__
  classDef decision fill:#fff4e5,stroke:#d68a1d,color:#1f2328
  classDef llm fill:#eef3ff,stroke:#3b6fd6,color:#1f2328
  classDef flow fill:#fafbfc,stroke:#9aa1ab,color:#1f2328
  classDef terminal fill:#ffffff,stroke:#57606a,color:#1f2328
  class security__assess decision
  class security__confirm llm
  class security flow
