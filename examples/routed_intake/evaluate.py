"""Evaluate deterministic routing at pipeline, flow, and decision-step scope."""
...
from examples.routed_intake.run import CONFIG_PATH, open_example
    ...
    async with open_example(live=live, prepared=prepared) as app:
        ...
        for spec in gold.suites:
            ...
            async def invoke(
                ...
                if step is not None:
                    return await app.run_step("routed_intake", flow or "classify", step, envelope)
                if flow is not None:
                    return await app.run_flow("routed_intake", flow, envelope)
                return await app.run("routed_intake", envelope)
