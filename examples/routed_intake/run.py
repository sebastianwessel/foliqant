"""Run a decision followed by an exact deterministic flow route."""
...
from examples.routed_intake import offline
...
async def open_example(
    *,
    ...
    """Compile handlers once and open the scripted or configured model."""
...
async def run_example(payload: dict[str, JsonValue], *, live: bool = False) -> ExecutionResult:
    async with open_example(live=live) as app:
        return await app.run("routed_intake", Envelope(payload=payload))
