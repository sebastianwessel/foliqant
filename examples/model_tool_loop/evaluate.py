"""Evaluate a model-to-tool-to-model loop with committed synthetic gold."""
...
from examples.model_tool_loop.run import CONFIG_PATH, open_example, runtime_environment
        ...
        for spec in gold.suites:
            ...
            async def invoke(
                ...
                if selected_flow is None:
                    return await app.run("request_assistant", envelope)
                if selected is None:
                    return await app.run_flow("request_assistant", selected_flow, envelope)
                return await app.run_step("request_assistant", selected_flow, selected, envelope)
