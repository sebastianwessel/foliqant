...
async def test_default_evidence_example_never_opens_a_provider(monkeypatch):
    ...
    assert (await example.run_evaluations(validation=True))["cases"] == 12


def test_evidence_gold_has_valid_targets_and_family_isolation():
    prepared = prepare_application(example.CONFIG_PATH)
    step = prepared.plans["decision_evidence"].flow("assessment").step("assess")
    ...
    development_cases = development.suites[0].gold_cases
    validation_cases = validation.suites[0].gold_cases
    assert {case.input.metadata.model_dump()["family"] for case in development_cases}.isdisjoint(
        case.input.metadata.model_dump()["family"] for case in validation_cases
    )
    assert {case.input.payload["message"] for case in development_cases}.isdisjoint(
        case.input.payload["message"] for case in validation_cases
    )
    assert len(development.suites[0].gold_cases) == 15
    assert len(validation.suites) == 3
...
def test_isolated_predicate_reuses_grouped_criterion_source_and_gold():
    prepared = prepare_application(example.CONFIG_PATH)
    grouped = prepared.plans["decision_evidence"].flow("assessment").step("assess")
    isolated = prepared.plans["decision_predicate"].flow("assessment").step("assess")
    ...
    for case in gold.suites[3].gold_cases:
...
async def test_request_count_scorer_rejects_additional_units_without_generated_id_gold():
    ...
    assert example.build_suite(gold, gold.suites[3]) == gold.to_suite(gold.suites[3])
