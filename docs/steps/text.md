...
`max_iterations` is an optional step field, default 8 (1–1024), counting logical model turns including the final answer. A simple text response normally needs one turn; `max_iterations: 1` enforces that. Provider retries count against the separate request-attempt limit, described in [execution limits](../configuration/limits.md).

Provider refusal (`output_refused`), output cut off at `max_tokens` (`output_limit_reached`, see [output budget](llm.md#set-the-output-budget-and-reasoning-effort)), invalid output, timeouts and limits are technical failures with their own [codes](../integration/errors.md#canonical-error-codes); they fail the run and never become a review outcome. If the text must meet a strict business shape or completeness rule, use a JSON schema or a typed decision and evaluate the content with reviewed examples. An attached read-only MCP tool can turn this into a [bounded agent loop](agent-loops.md).
