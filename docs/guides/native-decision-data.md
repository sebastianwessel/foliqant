...
and exact model name in the ignored root `.env`;
[`.env.example`](https://github.com/sebastianwessel/foliqant/blob/main/.env.example)
...
The [full recipe](https://github.com/sebastianwessel/foliqant/blob/main/model/examples/native-full.yaml) selects up to 2,000
...
Use [extend and migrate decision runs](decision-run-maintenance.md) to add scoped
source projections, migrate a completed run to current deterministic rules, or
rerun a frozen pending queue. These operations create immutable children and
never relabel their parent.

Use [decision contracts in application code](decision-contracts.md) to define
category boundaries, validate answerability and evidence, and apply results
without treating explanations or model output as authorization.
